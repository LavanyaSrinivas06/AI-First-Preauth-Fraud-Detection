...
def review_close(
    ...
    """
    payload example:
    { "analyst_decision": "APPROVE", "analyst": "lavanya", "notes": "looks fine" }
    """
    ...
    if not analyst:
        raise ApiError(400, "missing_required_field", "Missing analyst", param="analyst")

    ok = update_review(settings.abs_sqlite_path(), review_id, analyst_decision, analyst, notes)
    if not ok:
        raise ApiError(404, "resource_missing", "Review not found.", param="review_id")
